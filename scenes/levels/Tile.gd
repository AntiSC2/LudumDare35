...
func _fixed_process(delta):
	...
	if (mouse_over == true && selected == false && army == false && Input.is_action_pressed("select") && GM.moves > 0 && GM.hold == false):
		GM.hold = true;
		if (_find_army() == true):
			army = true;
			GM.moves -= 1;
	...
	elif (selected == false && owned == true && mouse_over == true && Input.is_action_pressed("select") && GM.hold == false && GM.moves <= 0):
		...
		if (_find_neighbour() == true):
			GM.expands -= 1;
			owned = true;
	elif (Input.is_action_pressed("destroy") && selected == true):
		selected = false;
	elif (Input.is_action_pressed("select") == false && GM.hold == true):
		GM.hold = false;
	
	if (mouse_over == true && selected == false && army == true):
		get_node("Sprite").set_modulate(Color(0.4, 1, 0.4));
		...
		get_node("Sprite").set_modulate(Color(0.6, 0.6, 0.6));
	elif (mouse_over == true && selected == false && owned == true):
		get_node("Sprite").set_modulate(Color(1, 1, 1));
	elif (selected == true):
		get_node("Sprite").set_modulate(Color(0.1, 1, 0.1));
